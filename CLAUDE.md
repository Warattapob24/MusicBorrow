...
8. **pg_net is `net.http_post`, not `extensions.net.http_post`.** The extension's namespace is `extensions` but its functions live in schema `net` — verify with `SELECT nspname FROM pg_proc ... WHERE proname='http_post'`. Calling the wrong path raises, and if the call sits inside a plpgsql block with `EXCEPTION WHEN OTHERS`, the whole block silently rolls back. (`MIGRATION_PUSH_TRIGGERS.sql` still shows the wrong form.)
9. A bare `EXCEPTION WHEN OTHERS THEN RAISE WARNING` hides real bugs *and* rolls back every statement in that block. Always include `SQLSTATE` in the warning, and never let the handler cover writes you expect to survive.
